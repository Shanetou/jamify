import React, { useState } from "react";
import { useDispatch, useSelector } from "react-redux";

import Button from "@material-ui/core/Button";
import { default as MuiDialog } from "@material-ui/core/Dialog";
import DialogActions from "@material-ui/core/DialogActions";
import DialogContent from "@material-ui/core/DialogContent";
import DialogContentText from "@material-ui/core/DialogContentText";
import DialogTitle from "@material-ui/core/DialogTitle";

import { dialogSelector } from "../selectors";
import { DIALOGS } from "../constants";
import { hideErrorDialog } from "../redux/actions/ui";

export const Dialog = props => {
  const dialog = useSelector(dialogSelector);
  const dispatch = useDispatch();

  const handleClose = () => {
    dispatch(hideErrorDialog());
  };

  if (dialog !== null) {
    const isGenericError = dialog.dialogType === DIALOGS.GENERIC_ERROR;
    const isAuthenticationError =
      dialog.dialogType === DIALOGS.AUTHENTICATION_ERROR;

    if (isAuthenticationError) {
      return <AuthenticationErrorDialog close={handleClose} />;
    } else if (isGenericError) {
      return <GenericErrorDialog close={handleClose} />;
    }
  }

  return null;
};

export const AuthenticationErrorDialog = ({ close }) => {
  function handleClick() {
    close();
    window.location = "http://localhost:8888/login";
  }

  return (
    <MuiDialog open={true} disableBackdropClick={true}>
      <DialogTitle>{"Session has expired"}</DialogTitle>
      <DialogContent>
        <DialogContentText>Please refresh your session now.</DialogContentText>
      </DialogContent>
      <DialogActions>
        <Button onClick={handleClick} color="primary" autoFocus>
          Refresh Session
        </Button>
      </DialogActions>
    </MuiDialog>
  );
};
export const GenericErrorDialog = ({ close }) => {
  function handleClick() {
    close();
    window.location.reload();
  }

  return (
    <MuiDialog open={true}>
      <DialogTitle>{"Oops, something bad happened"}</DialogTitle>
      <DialogContent>
        <DialogContentText>Please refresh the page now.</DialogContentText>
      </DialogContent>
      <DialogActions>
        <Button onClick={handleClick} color="primary" autoFocus>
          Refresh page.
        </Button>
      </DialogActions>
    </MuiDialog>
  );
};
